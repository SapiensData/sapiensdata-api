...
CREATE PROCEDURE sp_CreateSubLabel @parent_label_id INT,
...
NOCOUNT ON;
    ...
    IF
NOT EXISTS (SELECT 1 FROM Label WHERE label_id = @parent_label_id)
BEGIN
        PRINT
...
END
    ...
    IF
EXISTS (SELECT 1 FROM Label WHERE label_name = @sub_label_name AND parent_label_id = @parent_label_id)
BEGIN
        PRINT
'Sub-label already exists under this parent label.';
        RETURN;
END
...
PRINT
'Sub-label created successfully.';
END;
GO
