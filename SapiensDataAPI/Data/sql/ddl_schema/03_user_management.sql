...
(
    50
) UNIQUE NOT NULL, -- Username for login
    password_hash NVARCHAR
(
    255
) NOT NULL, -- Hashed password for secure storage
    prefix NVARCHAR
(
    10
...
(
    ...
    profile_picture_path NVARCHAR
(
    255
), -- File path or URL to profile picture
    account_email NVARCHAR
(
    100
) UNIQUE, -- Primary account email
    recovery_email NVARCHAR
(
    100
), -- Email for account recovery
    account_phone NVARCHAR
(
    20
), -- Primary phone number for account
    recovery_phone NVARCHAR
(
    20
...
(
    255
), -- Comma-separated list of alternate emails with titles (e.g., "work: example@work.com, personal: example@home.com")
...
(
    ...
    app_language NVARCHAR
(
    10
) DEFAULT 'en', -- Preferred app language (e.g., 'en', 'de')
    website NVARCHAR
(
    255
...
(
    50
) DEFAULT '[User]', -- [User] role (e.g., '[User]', 'admin')
    status NVARCHAR
(
    20
) DEFAULT 'active', -- Account status (e.g., 'active', 'suspended')
    );
GO
